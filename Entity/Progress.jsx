import React, { useState, useEffect } from 'react';
import axios from 'axios';
import {
  Typography,
  Box,
  Paper,
  Stack,
  Snackbar,
  Alert,
  TextField,
  Button,
  Card,
  IconButton
} from '@mui/material';
import { Edit, Delete } from '@mui/icons-material';
import HomeSidebar from '../Components/HomeSidebar';

const Progress = () => {
  const [progressData, setProgressData] = useState([]);
  const [newProgress, setNewProgress] = useState({ weight: '', date: '' });
  const [isEditing, setIsEditing] = useState(false);
  const [progressIdToEdit, setProgressIdToEdit] = useState(null);
  const [snackbarOpen, setSnackbarOpen] = useState(false);
  const [snackbarMessage, setSnackbarMessage] = useState('');

  // Fetch progress data on component mount
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/progress');
        setProgressData(response.data);
      } catch (error) {
        console.error('Error fetching progress:', error);
        setSnackbarMessage('Failed to fetch progress!');
        setSnackbarOpen(true);
      }
    };
    fetchProgress();
  }, []);

  // Add new progress
  const addProgress = async () => {
    if (!newProgress.weight || !newProgress.date) {
      setSnackbarMessage('Please fill out all fields!');
      setSnackbarOpen(true);
      return;
    }

    try {
      const response = await axios.post('http://localhost:8080/api/progress', newProgress);
      setProgressData([...progressData, response.data]);
      setNewProgress({ weight: '', date: '' });
      setSnackbarMessage('Progress added successfully!');
      setSnackbarOpen(true);
    } catch (error) {
      console.error('Error adding progress:', error);
      setSnackbarMessage('Failed to add progress!');
      setSnackbarOpen(true);
    }
  };

  // Update existing progress
  const updateProgress = async () => {
    if (!newProgress.weight || !newProgress.date) {
      setSnackbarMessage('Please fill out all fields!');
      setSnackbarOpen(true);
      return;
    }

    try {
      const response = await axios.put(
        `http://localhost:8080/api/progress/${progressIdToEdit}`,
        newProgress
      );
      setProgressData(progressData.map((item) =>
        item.progressID === progressIdToEdit ? response.data : item
      ));
      setNewProgress({ weight: '', date: '' });
      setIsEditing(false);
      setProgressIdToEdit(null);
      setSnackbarMessage('Progress updated successfully!');
      setSnackbarOpen(true);
    } catch (error) {
      console.error('Error updating progress:', error);
      setSnackbarMessage('Failed to update progress!');
      setSnackbarOpen(true);
    }
  };

  // Delete progress
  const deleteProgress = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/progress/${id}`);
      setProgressData(progressData.filter((item) => item.progressID !== id));
      setSnackbarMessage('Progress deleted successfully!');
      setSnackbarOpen(true);
    } catch (error) {
      console.error('Error deleting progress:', error);
      setSnackbarMessage('Failed to delete progress!');
      setSnackbarOpen(true);
    }
  };

  // Handle input changes
  const handleChange = (e) => {
    const { name, value } = e.target;
    setNewProgress({ ...newProgress, [name]: value });
  };

  // Edit progress
  const handleEdit = (item) => {
    setNewProgress({ weight: item.weight, date: item.date });
    setIsEditing(true);
    setProgressIdToEdit(item.progressID);
  };

  return (
    <Box display="flex" height="100vh" fontFamily="Roboto, sans-serif" bgcolor="#fff">
      <HomeSidebar />

      {/* Main Content */}
      <Box flex={1} display="flex" flexDirection="row" px={10} py={5} bgcolor="#eee" boxShadow='0 4px 8px rgba(0, 0, 0, 0.1)' marginTop="100px" height="80vh" ml={30} >

        {/* Input Form */}
        <Box flex={1} mr={4}>
          <Typography
            variant="h4"
            fontWeight="600"
            color="#000"
            align="center"
            mb={3}
          >
            My Progress
          </Typography>

          <Paper
            elevation={3}
            sx={{
              p: 3,
              mb: 4,
              borderRadius: 2,
              backgroundColor: '#fff',
              maxWidth: '600px',
              minWidth: '300px',
            }}
          >
            <Stack spacing={2}>
              <TextField
                name="weight"
                label="Weight (kg)"
                type="number"
                value={newProgress.weight}
                onChange={handleChange}
                fullWidth
              />
              <TextField
                name="date"
                label="Date"
                type="date"
                value={newProgress.date}
                onChange={handleChange}
                fullWidth
                InputLabelProps={{ shrink: true }}
              />
              <Button
                variant="contained"
                onClick={isEditing ? updateProgress : addProgress}
                sx={{ mt: 2, py: 1.5, backgroundColor: '#002366' }}
              >
                {isEditing ? 'Update Progress' : 'Add Progress'}
              </Button>
            </Stack>
          </Paper>
        </Box>

        {/* Progress Display */}
        <Box width="400px" maxWidth="100%" mt={4}>
          <Paper
            elevation={2}
            sx={{
              bgcolor: '#002366',
              color: '#ffffff',
              p: 2,
              borderRadius: 2,
              height: '100%',
            }}
          >
            <Typography variant="h6" mb={2}>
              Progress Records
            </Typography>

            {progressData.length > 0 ? (
              <Stack spacing={2}>
                {progressData.map((item) => (
                  <Card
                    key={item.progressID}
                    sx={{
                      bgcolor: '#ffffff',
                      color: '#333',
                      p: 2,
                      borderRadius: 1,
                    }}
                  >
                    <Typography variant="body1"fontSize="25px" fontWeight="600" color="black">
                      Weight: {item.weight} kg
                    </Typography>
                    <Typography variant="body2" fontSize="15px" >
                      Date: {item.date}
                    </Typography>
                    <IconButton
                      size="small"
                      onClick={() => handleEdit(item)}
                      disableRipple
                      sx={{
                        justifyContent: 'right',
                        backgroundColor: 'transparent', // No background color
                        '&:hover': {
                            backgroundColor: 'transparent', // No hover background
                            color: 'inherit', // Keep the color as is while hovering
                        },
                        '&:active': {
                            backgroundColor: 'transparent', // Remove active background
                            boxShadow: 'none', // Remove active click shadow
                        },
                        '&:focus-visible': {
                            outline: 'none', // Remove the focus ring
                            boxShadow: 'none', // Remove any focus-related shadow
                        },
                        '&.MuiIconButton-root': {
                            padding: 0, // Ensure there is no padding around the icon
                            outline: 'none', // Force remove any focus outline
                            boxShadow: 'none', // Force remove shadow on focus/click
                        },
                    }}
                    >
                      <Edit />
                    </IconButton>

                    <IconButton
                      size="small"
                      color="error"
                      onClick={() => deleteProgress(item.progressID)}
                      disableRipple
                      sx={{
                        justifyContent: 'right',
                        backgroundColor: 'transparent', // No background color
                        '&:hover': {
                            backgroundColor: 'transparent', // No hover background
                            color: 'inherit',
                        },
                        '&:active': {
                            backgroundColor: 'transparent', // Remove active background
                            boxShadow: 'none', // Remove active click shadow
                        },
                        '&:focus-visible': {
                          outline: 'none', // Remove the focus ring
                          boxShadow: 'none', // Remove any focus-related shadow
                        },
                        '&.MuiIconButton-root': {
                            padding: 0, // Ensure there is no padding around the icon
                            outline: 'none', // Force remove any focus outline
                            boxShadow: 'none', // Force remove shadow on focus/click
                        },
                    }}
                    >
                      <Delete />
                    </IconButton>
                  </Card>
                ))}
              </Stack>
            ) : (
              <Typography variant="body2">No progress records added yet.</Typography>
            )}
          </Paper>
        </Box>
      </Box>

      {/* Snackbar */}
      <Snackbar
        open={snackbarOpen}
        autoHideDuration={4000}
        onClose={() => setSnackbarOpen(false)}
      >
        <Alert
          onClose={() => setSnackbarOpen(false)}
          severity={snackbarMessage.includes('Failed') ? 'error' : 'success'}
          sx={{ width: '100%' }}
        >
          {snackbarMessage}
        </Alert>
      </Snackbar>
    </Box>
  );
};

export default Progress;
